...
Public Class frmMainTransportUnit_main
    ...
    Private Sub InitialTransport()
        ...
        strSQL = _
             "select T.TU_ID,T.TU_NUMBER,T.TU_ID AS TuNAME " & _
    ...
    Function Show_Data_Search() As Boolean
        ...
        sql_str = _
        "select T.TU_NUMBER as ""รหัสรถ"",T.TU_ID as ""หมายเลขทะเบียนรถ"",T.LAST_CAL_DATE as ""วันวัดน้ำหมดอายุครั้งก่อน""," & _
        " T.NEXT_CAL_DATE as ""วันวัดน้ำหมดอายุ"",TARE_WEIGHT as ""น้ำหนักรถเปล่า"",case T.IS_ENABLED when 1 then 'สามารถใช้งานได้' else 'หยุดการใช้งาน' end as ""สถานะการใช้งาน"", T.COMPARTMENTS as ""จำนวนช่องเติม""," & _
        "T.TU_TYPE||'-'||TU.description AS ""ชนิดรถ"" ," & _
        "t.CAPACITYS as""ความจุ"", t.UPDATE_DATE as""วันที่แก้ไขล่าสุด"", t.UPDATED_BY as""แก้ไขโดย""" & _
        " from TRANSPORT_UNIT T,tas.view_tu_type TU" & _
        " where t.tu_type=tu.status_varchar(+)" & _
        " and T.TU_ID like  ('%" & cbTUHead.Text & "%')" & _
