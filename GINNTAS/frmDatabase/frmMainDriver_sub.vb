...
#End Region
    ...
    Private Sub UploadPicture()
        Dim picdriver As Byte()
        Dim ms As New IO.MemoryStream
        If (txtPathPicDriver.Text.ToString() <> String.Empty) Then
            picdriver = ReadAllBytes(txtPathPicDriver.Text)
            If frm_work = 1 Then 'Add
                Oradb.UploadImagePicture(1, txtDriverID.Text, 1, picdriver, mComputerName, mUserName)
            Else
                Oradb.UploadImagePicture(2, txtDriverID.Text, 1, picdriver, mComputerName, mUserName)
            End If
