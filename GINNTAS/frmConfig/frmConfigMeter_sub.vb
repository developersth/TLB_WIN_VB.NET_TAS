...
#End Region
    ...
    Private Sub Save()
        ...
        "insert into METER(" & _
        " METER_NO,METER_ADDRESS,PRES_LIMIT_HIGH, " & _
        " METER_TYPE,BASE_PRODUCT_ID,ARM_LEFT_ACTIVE,ARM_RIGHT_ACTIVE,ISLAND_NO,MAP_ID, " & _
        " COMP_ID,COMPORT_NO ,COMP_ID1,COMPORT_NO1,IS_ENABLED,IS_LOCKED,BATCH_ID,CREATION_DATE,CREATED_BY, " & _
        " UPDATE_DATE,UPDATED_BY, PULSE_INPUT,PRES_ALARM_HH,PRES_ALARM_H) " & _
        " values('" & _
        Trim(txtMeterNo.Text) & "','" & Trim(txtMeterAddr.Text) & "','" & Trim(txtPressureDiff.Text) & "','" & _
        Trim(cbMeterType.Text) & "','" & Trim(cbBaseProduct.Text) & "','" & IIf(chkArmPos.Checked = True, 1, 0) & "','" & _
        IIf((ChkArmPos2.Checked = True), 1, 0) & "','" & Trim(cbIslandNo.Text) & "','" & Trim(cbMapID.Text) & "','" & _
        Trim(CompId) & "','" & Trim(CompNo) & "','" & _
        Trim(sCompId2) & "','" & Trim(sCompNo2) & "'," & _
        IIf(OptEnabled.Checked, 1, 0) & "," & _
        IIf(OptLock.Checked, 1, 0) & ",'1'," & _
    ...
    Private Sub Edit()
        ...
        "update METER set " & _
        " METER_NO='" & Trim(txtMeterNo.Text) & "', " & _
        " METER_ADDRESS=" & Trim(txtMeterAddr.Text) & ", " & _
        " METER_TYPE='" & Trim(cbMeterType.Text) & "', " & _
        " BASE_PRODUCT_ID='" & Trim(cbBaseProduct.Text) & "', " & _
        " ARM_LEFT_ACTIVE=" & IIf((chkArmPos.Checked), 1, 0) & ", " & _
        " ARM_RIGHT_ACTIVE=" & IIf((ChkArmPos2.Checked), 1, 0) & ", " & _
        " ISLAND_NO='" & Trim(cbIslandNo.Text) & "', " & _
        " COMP_ID='" & Trim(CompId) & "'," & _
        " COMPORT_NO='" & Trim(CompNo) & "'," & _
        " COMP_ID1='" & Trim(sCompId2) & "'," & _
        " COMPORT_NO1='" & Trim(sCompNo2) & "'," & _
        " IS_ENABLED=" & IIf(OptEnabled.Checked, 1, 0) & ", " & _
        " IS_LOCKED=" & IIf(OptLock.Checked, 1, 0) & ", " & _
        "UPDATE_DATE=sysdate," & _
        "UPDATED_BY='" & mUserName & "', " & _
        "BLEND_PERCENT='" & Trim(txtMblendPercent.Text) & "' ," & _
        "PRES_LIMIT_HIGH='" & Trim(txtPressureDiff.Text) & "' ," & _
        "PRES_ALARM_HH='" & Trim(txtPressureHH.Text) & "' ," & _
        "PRES_ALARM_H='" & Trim(txtPressureH.Text) & "' ," & _
        " PULSE_INPUT = '" & indexCmbPulsInput & "'" & _
    ...
    Private Sub AssignValue()
        ...
        sql_str = _
        " select METER_NO,MAP_ID,METER_ADDRESS,METER_TYPE,METER_POSITION,ARM_TYPE,ARM_LEFT_ACTIVE,ARM_RIGHT_ACTIVE,COMP_ID||' ' ||COMPORT_NO as COMPORT_NO, " & _
        "  PRES_ALARM_H,PRES_ALARM_HH,PULSE_INPUT ,PRES_LIMIT_HIGH ,PRES_ALARM_H,COMP_ID1||' ' ||COMPORT_NO1 as COMPORT_NO1,BASE_PRODUCT_ID,MAP_ID,ISLAND_NO,IS_ENABLED,IS_LOCKED,UPDATE_DATE,UPDATED_BY ,BLEND_PERCENT" & _
        " from METER " & _
