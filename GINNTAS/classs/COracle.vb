...
#End Region
    ...
    Private Sub Reconnect()
        If Not oraConnect Then
            Try
                If (oraConnect) Then oraConn.Close()
            Catch e As Exception
            End Try
            Connect()
        End If
    ...
    Public Function OpenDys(ByVal strSQL As String, ByVal pTableName As String, ByRef mDataSet As DataSet,
        ...
        Return bCheck
    End Function
    Public Function ExecuteReader(ByVal strSQL As String, Optional ByRef SQL_Execution_Error As String = "", Optional ByVal pShowErr As Boolean = True) As DataTable
        Dim ds As New DataSet
        Dim bCheck As Boolean = False
        ...
        If oraConnect Then
            Try
                Dim cmd As OracleCommand = New OracleCommand(strSQL, oraConn)
    ...
    Public Function UploadImagePicture(Work_ID As Integer, PICTURE_ID As String, ByVal PICTURE_TYPE As String, byteimg As Byte(), SYSTEM_COMPUTER As String, SYSUSER As String) As Boolean
        ...
        If oraConnect Then
            Try
                ...
                strSQL = " SELECT COUNT(*) FROM PICTURE_TAS WHERE PICTURE_ID=" + PICTURE_ID
                If Not (Work_ID.Equals(1)) Then
                    strSQL = "UPDATE TAS.PICTURE_TAS SET UPDATE_DATE=sysdate,UPDATED_BY='" & SYSUSER & "',J_COMPUTER='" & SYSTEM_COMPUTER & "',PICTURE_DATA=:PICTURE_DATA WHERE  PICTURE_ID=" & PICTURE_ID

                    Oraparam.OracleDbType = OracleDbType.Blob
                    Oraparam.ParameterName = "PICTURE_DATA"
                    Oraparam.Value = byteimg
    ...
    Public Function ConnectServiceName() As String
        ...
        Select Case currentDB
            Case DB_TYPE.DB_MASTER
                oraServiceName = "Server A"
                If (oraConnect) Then oraServiceName = oraServiceName & " [" & oraConn.DatabaseName & "]"
            Case DB_TYPE.DB_BACKUP
                oraServiceName = "Server B"
                If (oraConnect) Then oraServiceName = oraServiceName & " [" & oraConn.DatabaseName & "]"
            Case Else
