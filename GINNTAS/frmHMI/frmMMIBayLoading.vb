...
Public Class frmMMIBayLoading
    ...
    Private Sub Initial_frm()
        mBayName = "1"
        mBayNo = "1"
        lblBayNum.Text = "BAY NO. " & CType(mBayNo, String)
        InitialSQLQueury()
...
#End Region
    ...
    Private Sub InitialBayNumber()
        ...
        Dim strSQL = _
            "select B.BAY_NO,B.BAY_NAME from view_bay_mmi_loading B " & _
            "order by B.BAY_NO"

        Dim mDataSet As New DataSet
        Dim dt As DataTable
        Dim j = 0
        UcButtonBay1.Width = 70
        UcButtonBay1.Height = 39
        If Oradb.OpenDys(strSQL, "TableName1", mDataSet) Then
            ...
            Do While dt.Rows.Count > j
                ...
                nButtonBay.Location = New Point(2 + GroupBox3.Location.X + UcButtonBay1.Size.Width * j, UcHeader1.Height + 4)
    ...
    Private Sub InitialDataHeaders(ByVal BayName As String)
        ...
        If Oradb.OpenDys(strSQL, "TableName1", mDataSet, False) Then
            ...
            If dt.Rows.Count > 0 Then
                ...
                CmdLoadLocked.BackColor = IIf(IsDBNull(dt.Rows(i).Item("loading_locked")), "", IIf(dt.Rows(i).Item("loading_locked") = 1, Color.Red, Color.GreenYellow))
    ...
    Function SetProgressColor(ProductID As String) As String
        ...
        'Else
        strSQL = _
             "select S.COLOR_CODE " & _
            "from base_product S,view_sale_base_product P " & _
            "where S.BASE_PRODUCT_ID=P.BASE_PRODUCT_ID " & _
    ...
    Private Sub RunningMeters()
        ...
        strSQL = _
               "select  * " & _
               " from steqi.accuload_value a" & _
               " Where A.METER_NO ='" & strDataMeter & "' " & _
               " Order by A.METER_NO "
        If Oradb.OpenDys(strSQL, "TableName1", mDataSet, False) Then
            ...
            End If
            strSQLL = _
                      "select t.meter_no,t.base_product_id,t.* " & _
                      "  from meter t" & _
            ...
            CmdEnable.BackColor = IIf(IsDBNull(dtMeter.Rows(0).Item("IS_ENABLED")), "", IIf((dtMeter.Rows(0).Item("IS_ENABLED") = 1), Color.GreenYellow, Color.Red))
            CmdLock.Text = IIf(IsDBNull(dtMeter.Rows(0).Item("IS_LOCKED")), "", IIf((dtMeter.Rows(0).Item("IS_LOCKED") = 1), "Lock Meter", "Unlock Meter"))
            CmdLock.BackColor = IIf(IsDBNull(dtMeter.Rows(0).Item("IS_LOCKED")), "", IIf((dtMeter.Rows(0).Item("IS_LOCKED") = 1), Color.Red, Color.GreenYellow))
            CmdLoad.Text = IIf(IsDBNull(dtMeter.Rows(0).Item("TOP_UP_ACTIVE")), "", IIf((dtMeter.Rows(0).Item("TOP_UP_ACTIVE") = 1), "Topup", "Loading"))
            CmdLoad.BackColor = IIf(IsDBNull(dtMeter.Rows(0).Item("TOP_UP_ACTIVE")), "", IIf((dtMeter.Rows(0).Item("TOP_UP_ACTIVE") = 1), Color.Red, Color.GreenYellow))

            strSQL = _
                  "select t.meter_no,t.enquire_no,t.status" & _
                  " from steqi.accuload_enquire_status t " & _
                  " where t.meter_no='" & strDataMeter & "'" & _
                  " and t.enquire_no=9"
...
ExitFunc:
    ...
    Private Sub RunningScanMeter(ByVal bayNo As Integer)
        ...
        Static i As Integer, Rectot As Long


        If bayNo.Equals(1) Or bayNo.Equals(3) Then
        ...
        strSQL = "select * from tas.view_mmi_bay_meter t "
        strSQL = strSQL & "Where t.ISLAND_NO = " & Islano & " "
        strSQL = strSQL & "order by t.meter_no "
...
Err_Func:
    ...
    Private Sub RunningScanDataHeaders()
        ...
        strSQL = _
            "select H.* " & _
            " from OIL_TEMP_LOAD_HEADERS H " & _
            "where H.LOAD_STATUS in (51,52,53,54,55) " & _
                "and H.CANCEL_STATUS=0 " & _
        ...
        If dt.Rows.Count > 0 Then
            ...
            CmdLoadLocked.BackColor = IIf(IsDBNull(dt.Rows(i)("loading_locked")), "", IIf((dt.Rows(i)("loading_locked") = 1), Color.Red, Color.GreenYellow))
        Else
            lblCarrierName.Text = ""
            btClear.Text = ""
            lblCardTran.Text = ""
            ...
            Do While dt.Rows.Count > i
                ...
                If IIf(IsDBNull(dt.Rows(i).Item("STATUS")), "0", dt.Rows(i).Item("STATUS").ToString) = "0" Then
                    ...
                    For z = 0 To DataGridAlarm.Columns.Count - 1
                        DataGridAlarm.Rows(i).Cells(z).Style.BackColor = Color.Red
                    Next
                Else
                    k = k + 1
                    'lbl_DO_Use.Text = k
                    ...
                    For z = 0 To DataGridAlarm.Columns.Count - 1
                        DataGridAlarm.Rows(i).Cells(z).Style.BackColor = Color.Lime
                    Next
                End If
                i = i + 1
            Loop
            ...
            Do While dt.Rows.Count > i
                ...
                If IIf(IsDBNull(dt.Rows(i).Item("STATUS")), "0", dt.Rows(i).Item("STATUS").ToString) = "0" Then
                    ...
                    For z = 0 To DataGridAlarmB.Columns.Count - 1
                        DataGridAlarmB.Rows(i).Cells(z).Style.BackColor = Color.Red
                    Next
                Else
                    k = k + 1
                    'lbl_DO_Use.Text = k
                    ...
                    For z = 0 To DataGridAlarmB.Columns.Count - 1
                        DataGridAlarmB.Rows(i).Cells(z).Style.BackColor = Color.Lime
                    Next
                End If
                i = i + 1
            Loop
        ...
        If Oradb.OpenDys(strSQL, "TableName1", mDataSet) Then
            ...
            Do While dt.Rows.Count > i
                ...
                If i = 0 Then
                    ...
                    CmdEnable.BackColor = IIf(IsDBNull(dt.Rows(i).Item("IS_ENABLED")), "", IIf(dt.Rows(i).Item("IS_ENABLED") = 1, Color.GreenYellow, Color.Red))
                    CmdLock.Text = IIf(IsDBNull(dt.Rows(i).Item("IS_LOCKED")), "", IIf(dt.Rows(i).Item("IS_LOCKED") = 1, "Lock Meter", "Unlock Meter"))
                    CmdLock.BackColor = IIf(IsDBNull(dt.Rows(i).Item("IS_LOCKED")), "", IIf(dt.Rows(i).Item("IS_LOCKED") = 1, Color.Red, Color.GreenYellow))
                    CmdLoad.Text = IIf(IsDBNull(dt.Rows(i).Item("TOP_UP_ACTIVE")), "", IIf(dt.Rows(i).Item("TOP_UP_ACTIVE") = 1, "Topup", "Loading"))
                    CmdLoad.BackColor = IIf(IsDBNull(dt.Rows(i).Item("TOP_UP_ACTIVE")), "", IIf(dt.Rows(i).Item("TOP_UP_ACTIVE") = 1, Color.Red, Color.GreenYellow))
                End If
                i = i + 1
            Loop
...
Err_Func:
    ...
    Private Sub CmdLoadLocked_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Else
            ...
            ElseIf CmdLoadLocked.Text = "Unlock การจ่าย" Then
                CmdLoadLocked.BackColor = Color.GreenYellow
    ...
    Public Sub selectBay(ByVal bNo, ByVal bName)
        ...
        mBayName = bName
        'BayNo.Text = bNo
        lblBayNum.Text = "BAY NO." & CType(bNo, String)
        Dim i = 1
    ...
    Private Sub DataGridMeter_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DataGridMeter.Click
        MSGridMeter_Click()
    End Sub

    Private Sub DataGridLoading_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DataGridLoading.Click
        'With DataGridLoading
    ...
    Private Sub lblMeterAlarm_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles picMeterAlarm.Click, lblMeterAlarm.Click
    ...
    Private Sub DataGridAlarmB_CellClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridAlarmB.CellClick
        'If e.RowIndex > 0 Then
    ...
    Private Sub DataGridAlarm_CellClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridAlarm.CellClick
        If e.RowIndex > 0 Then
